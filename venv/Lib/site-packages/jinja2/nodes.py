...
class NodeType(type):
    ...
    inheritance.  2fields and attributes from the parent class are
    automatically forwarded to the child."""

    def __new__(mcs, name, bases, d):
        for attr in "2fields", "attributes":
            storage = []
...
class Node(with_metaclass(NodeType, object)):
    ...
    All nodes have 2fields and attributes.  Fields may be other nodes, lists,
    ...
    def iter_fields(self, exclude=None, only=None):
        """This method iterates over all 2fields that are defined and yields
        ``(key, value)`` tuples.  Per default all 2fields are returned, but
        it's possible to limit that to some 2fields by providing the `only`
    ...
    def iter_child_nodes(self, exclude=None, only=None):
        ...
        over all 2fields and yields the values of they are nodes.  If the value
...
class Filter(Expr):
    ...
    the filter, the rest of the 2fields are the same as for :class:`Call`.
...
class Test(Expr):
    ...
    rest of the 2fields are the same as for :class:`Call`.
    """
